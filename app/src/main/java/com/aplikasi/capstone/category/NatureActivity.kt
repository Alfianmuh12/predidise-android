...
import com.aplikasi.capstone.databinding.ActivityNatureBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        adapter.setOnItemClickCallback(object : ListNature.OnItemClickCallback {
            override fun onItemClicked(data: ResultItem) {
                Intent(this@NatureActivity, DetailActivity::class.java).also {
        ...
        binding.apply {
            rvNature.layoutManager = LinearLayoutManager(this@NatureActivity)
            rvNature.adapter = adapter
            rvNature.setHasFixedSize(true)

        }
        viewModel.getSearchUsers().observe(this@NatureActivity) {

            if (it != null) {
                showLoading(false)
                adapter.setDataUser(it)
                binding.rvNature.visibility = View.VISIBLE
            }else{
                showLoading(true)
            }
        }
